...
const Card: React.FC<CardProps> = ({ title, icon, children, className = '', titleClassName = '', actions, footer, titleRightElement }) => {
  const clonedIcon = icon ? React.cloneElement(icon as React.ReactElement<{ size?: number; className?: string }>, { size: 24, className: `mr-2.5 text-sky-600 ${(icon.props as any)?.className || ''}` }) : null;
