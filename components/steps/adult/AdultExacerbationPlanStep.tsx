...
import { AlertTriangle, CheckCircle, ChevronRight, FileText, RotateCcw, ListPlus, ShieldAlert } from 'lucide-react';
...
const AdultExacerbationPlanStep: React.FC = () => {
  ...
  const cardIcon = isMildModerate ? <CheckCircle className="text-amber-600" /> : <ShieldAlert className="text-red-600" />;
    ...
    >
      ...
      <div className="mt-8 pt-6 border-t border-slate-200 space-y-3">
        <p className="text-xs text-slate-500 text-center">
            S'assurer que le patient dispose d'un plan d'action écrit pour l'asthme à jour. Revoir les déclencheurs d'exacerbation et les stratégies de prévention après la résolution de l'épisode.
        </p>
        <Button 
            onClick={() => navigateTo('ADULT_FOLLOW_UP_STEP')}
            variant="primary"
            fullWidth
            size="lg"
            leftIcon={<FileText />}
